            ...
            <div class="d-flex flex-column flex-md-row gap-3 align-items-center w-100">
                <div class="search-bar-wrapper flex-grow-1 w-100 d-flex align-items-center">
                    <form action="{{ url()->current() }}" method="get" class="flex-grow-1 w-100 d-flex align-items-center">
                        ...
                        <button type="submit" class="border-0 bg-transparent p-0"><i
                                class="bi bi-search text-muted fs-5"></i></button>
                    </form>
                </div>
            ...
            <div class="col-12 col-md-3 col-xl-2">
                <div class="sidebar-pendaftar h-100">
                    ...
                    <div class="d-flex flex-column">
                        {{-- Tombol Semua (Hapus filter status) --}}
                        <a href="{{ request()->fullUrlWithQuery(['status' => null, 'page' => 1]) }}"
                            class="tab-box {{ !request('status') || request('status') == 'all' ? 'active' : 'inactive' }} text-decoration-none">
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['status' => 'pending', 'page' => 1]) }}"
                            class="tab-box {{ request('status') == 'pending' ? 'active' : 'inactive' }} text-decoration-none">
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['status' => 'rejected', 'page' => 1]) }}"
                            class="tab-box {{ request('status') == 'rejected' ? 'active' : 'inactive' }} text-decoration-none">
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['status' => 'approved', 'page' => 1]) }}"
                            class="tab-box {{ request('status') == 'approved' ? 'active' : 'inactive' }} text-decoration-none">
                            <i class="bi bi-check-circle-fill"></i>
                            <span>Terverifikasi ({{ $totalApproved }})</span>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 col-md-9 col-xl-10">
                <div class="table-responsive pb-2">
                    <table class="table-custom">
                        ...
                        <tbody id="table-body">
                        @php $i = $documents->firstItem() ?? 1; @endphp
                        
                        {{-- 2. Ubah $data menjadi $documents --}}
                        @foreach ($documents as $team)
                        <tr class="table-row">
                            <td>{{ $i++ }}</td>
                            ...
                            <td>
                                <div class="badge-status {{ $team->status_document == 'approved' ? 'bg-verified' : 'bg-pending' }} shadow-sm">
                                    ...
                                    {{ ucfirst($team->status_document) }}
                                </div>
                            </td>
                            <td>
                                <div class="action-icons">
                                    {{-- Tombol Edit Modal (Sama seperti sebelumnya) --}}
                                    <button class="icon-action icon-edit btn-edit" 
                                        ...
                                        data-ketua="{{ $team->ketua_name ?? 'Belum ada ketua' }}" 
                                        data-status="{{ $team->status_team ? 'Terverifikasi' : 'Pending' }}"
                                        data-pdf="{{ $team->document_path ? asset('storage/' . $team->document_path) : '#' }}">
                                        <i class="bi bi-pencil-square"></i>
                                    </button>
                                    
                                    <a href="/admin/deleteTeam/{{ $team->team_id }}" class="icon-action icon-delete" onclick="return confirm('Yakin ingin menghapus tim ini?')">
                ...
                <div class="modal-body p-4 p-md-5">
                    <div class="row g-4 align-items-center">
                        ...
                        <div class="col-md-5">
                            ...
                            <a href="#" target="_blank" id="modal-pdf-link"
                                class="pdf-placeholder text-decoration-none w-100">
                                <i class="bi bi-filetype-pdf" style="font-size: 4rem; color: #D81E76;"></i>
                                <span class="fw-bold mt-2"
                                    style="color: #D81E76; font-size: 1.2rem; letter-spacing: 2px;">Buka PDF</span>
                            </a>
                        </div>
                        ...
                        <div class="col-md-7 ps-md-4">
                            ...
                            <div class="d-flex gap-3 mt-4">
                                {{-- TOMBOL TOLAK --}}
                                <button class="btn btn-modal-outline w-50 py-2 fw-bold" data-bs-toggle="modal"
                                    data-bs-target="#modalTolak">
                                    Tolak
                                </button>

                                {{-- FORM VERIFIKASI (SIAP DISAMBUNGKAN KE ROUTE) --}}
                                <form id="form-verifikasi" method="POST" action="{{ route('admin.updateStatus') }}"
                                    class="w-50">
                                    @csrf
            ...
            <div class="modal-content border-0 rounded-4 overflow-hidden shadow">
                ...
                <div class="modal-body p-4 p-md-5">
                    ...
                    <form id="form-tolak" method="POST" action="{{ route('admin.rejectDocument') }}">
                        ...
                        <textarea name="alasan_penolakan" class="form-control bg-light-custom border-0 rounded-3 mb-4 p-3"
                            rows="5" placeholder="Tuliskan alasan penolakan..." required></textarea>

                        <button type="submit"
                            class="btn btn-modal-gradient w-100 py-2 fw-bold d-flex justify-content-center align-items-center gap-2">
            ...
            <div class="modal-content border-0 rounded-4 p-4 p-md-5 text-center shadow">
                ...
                <div class="mb-4 mt-2">
                    <div class="d-inline-flex align-items-center justify-content-center rounded-circle"
                        style="width: 80px; height: 80px; background-color: #E8F8F5;">
                ...
                <p class="text-muted small mb-4 px-2">Team <span id="modal-success-team-name"></span> Berhasil di Verifikasi
                    Admin</p>
